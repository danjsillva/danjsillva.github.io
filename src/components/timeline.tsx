    ...
    <section className="flex flex-col justify-end w-[900px] h-full mt-40">
      {events.map((event) => (
        <article key={event.title} className="flex">
          <div className="w-60 h-auto p-8 pt-16 text-right border-r border-zinc-900">
            <span className="">{event.date}</span>
          </div>

          <div className="w-6 h-6 mt-16 bg-zinc-900 rounded-full -ml-3"></div>

          <div className="flex-1 p-8 m-8 border border-zinc-900 rounded">
            <header>
            ...
            <div className="flex flex-wrap gap-1 mt-4">
              {event.tags?.map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1 text-sm bg-zinc-900 text-white bg-black rounded-full"
                >
                  {tag}
                </span>
              ))}
